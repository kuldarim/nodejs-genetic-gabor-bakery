...
chromosome.prototype.mutate = function() {
  const row = number.getRandomIntInclusive(this.min, this.imageSize);
  const index = number.getRandomIntInclusive(this.min, this.imageSize);
  const randomValue = row + '-' + index;

  const values = [...this.values];
  // TODO maybe should random on values which are not existing yet
  values.push(randomValue);
  return new chromosome(lodash.uniq(values).sort().slice(0, this.max + 1));
}
